...
from models.network_origin import SwinIR as net
...
def final_test(input_tmp, model):
    # outputs = []
    # sf_ = torch.Tensor(2.,2.)
    # for k in range(0,8,1):
    #     test_input = torch.rot90(input_tmp, k, (2,3)) if k < 4 else torch.fliplr(torch.rot90(input_tmp, k, (2,3)))

    #     interpolated_tmp = test_input
    #     # interpolated_tmp = torch.Tensor(interpolated_tmp.copy())
    #     interpolated_tmp = (torch.Tensor(interpolated_tmp.copy()).permute(2,0,1)).unsqueeze_(0)
    #     interpolated_tmp = interpolated_tmp.cuda()

    #     # Apply network on the rotated input
    #     tmp_output = np.squeeze(model(interpolated_tmp).cpu().detach().permute(0,2,3,1).numpy())
    ...
    #         tmp_output = back_projection(tmp_output, input_tmp, down_kernel='cubic',
    #                                         up_kernel='cubic', sf=sf_)
    ...
    # for bp_iter in range(10):
    #     almost_final_sr = back_projection(almost_final_sr, input_tmp, down_kernel='cubic',
    #                                         up_kernel='cubic', sf=sf_)
    ...
    # # final_sr = almost_final_sr
    # final_sr = torch.Tensor(almost_final_sr).permute(2,0,1).unsqueeze_(0)
    ...
    input_tmp = np.squeeze(np.transpose(input_tmp,(0,2,3,1)))
    outputs = []
    sf_ = np.array([2.,2.])
    for k in range(0, 8, 1):
        # Rotate 90*k degrees and mirror flip when k>=4
        test_input = np.rot90(input_tmp, k) if k < 4 else np.fliplr(np.rot90(input_tmp, k))
...
def main():
    ...
    # for i in range(3):
    #     for j in range(14):
    #         define_model_path = '/home/shiminghao/smh/ckptallpic-epoch3/checkpoint_%d_%d.pth' % (i,j)
        ...
        for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
            ...
            with torch.no_grad():
                ...
                output = final_test(img_lq.cpu().detach().numpy(), model)
...
def define_model(args):
    ...
    # pretrained_model = torch.load(define_model_path)
