...
# from models.network_modified_elab import SwinIR as net
from models.network_mbconv_elab_swinir import SwinIR as net
...
def define_model(args):
    ...
    # use 'pixelshuffledirect' to save parameters
        ...
        model = net(upscale=args.scale, in_chans=3, img_size=64, window_size=16,
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        # print(imgname)
        # img_lq = cv2.imread(f'{args.folder_lq}/lr{imgext}', cv2.IMREAD_COLOR).astype(

        img_lq = cv2.imread(f'{args.folder_lq}/{imgname}x{args.scale}{imgext}', cv2.IMREAD_COLOR).astype(
        # img_lq = cv2.imread(f'{args.folder_lq}/{imgname[:-3]}{imgext}', cv2.IMREAD_COLOR).astype(
