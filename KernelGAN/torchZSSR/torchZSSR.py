...
class ZSSR:
    ...
    def __init__(self, input_img_path, ground_img_path, scale_factor=2, kernels=None, is_real_img=False, noise_scale=1.):
        ...
        ##the extra part###
        self.ground_truth = img.imread(ground_img_path)
        self.ground_truth = self.ground_truth / 255. if self.ground_truth.dtype == 'uint8' else self.ground_truth # ???
    ...
    def run(self):
        ...
        for self.sf_ind, (sf, self.kernel) in enumerate(zip(self.conf.scale_factors, self.kernels)):
            ...
            #####the extra part#######
            scale=int(self.conf.scale_factors[0][0])
            PSNR=psnr(rgb2y(np.round(np.clip(self.ground_truth*255., 0.,255.)).astype(np.uint8))[scale:-scale, scale:-scale],
                  rgb2y(np.round(np.clip(post_processed_output*255., 0., 255.)).astype(np.uint8))[scale:-scale, scale:-scale])
